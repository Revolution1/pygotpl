...
from gotime.go import Location, LocationError, Time
...
def test_tzif_parser_rejects_invalid_headers_and_indexes() -> None:
...
def test_zone_data_loading_supports_system_and_packaged_paths(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    ...
    monkeypatch.setattr(_tzfile.zoneinfo, "TZPATH", (str(tmp_path),))
    assert (_tzfile._read_zone_data("America/New_York") or b"").startswith(b"TZif")
    monkeypatch.setattr(_tzfile.zoneinfo, "TZPATH", ())
    assert (_tzfile._read_zone_data("America/New_York") or b"").startswith(b"TZif")
...
def test_location_can_be_loaded_directly_from_tzif_data() -> None:
    location = Location.from_tzdata(
        "Example/Fixed",
        _tzif_v1(abbreviation=b"ABC\0", transition_type=0),
    )
    value = Time.from_unix(1, location=location)

    assert value.zone() == ("ABC", 3_600)
    start, end = value.zone_bounds()
    assert start.equal(Time.from_unix(0))
    assert end.is_zero()


def test_location_from_tzdata_rejects_malformed_data() -> None:
    with pytest.raises(LocationError, match="malformed time zone information"):
        Location.from_tzdata("Broken", b"not tzif")
    with pytest.raises(TypeError, match="data must be bytes"):
        Location.from_tzdata("Broken", "not bytes")  # type: ignore[arg-type]


def test_iana_abbreviation_and_transition_edge_paths() -> None:
    new_york = ZoneInfo("America/New_York")
    winter = datetime(2024, 1, 15, 12)

    assert _tzfile.abbreviation_offset(new_york, "EST", winter) == -18_000
    assert _tzfile.zone_before_first_transition(new_york, -(1 << 40)) is not None
    assert _tzfile.transition_bounds(UTC, 0) == (None, None)
    assert _tzfile.transition_bounds(new_york, 0, transitions=()) == (None, None)

    metadata = _tzfile._ZoneMetadata(
        (0,),
        (1,),
        (
            _tzfile._ZoneType("A", 0, False),
            _tzfile._ZoneType("B", 1, False),
        ),
    )
    assert _tzfile._first_zone_index(metadata) == 0
