...
| Automated historical tracking | covered | `performance-history.yml` runs sampled render, parser, async, and memory benchmarks on relevant `main` changes, weekly, and manually; revision-named JSON artifacts are retained for 90 days. The workflow YAML parses locally and every artifact-producing CLI has smoke coverage or a real local run. |
